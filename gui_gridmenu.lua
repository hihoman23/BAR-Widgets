...
local function updateQuotaNr(unitDefID, count)
	...
	if WG.Quotas then
		...
		quotas[unitDefID] = quotas[unitDefID] or {amount = 0}
		quotas[unitDefID].amount = math.max(quotas[unitDefID].amount + (count or 0), 0)
		WG.Quotas.update(quotas)
		cellRect.opts.quotanr = quotas[unitDefID].amount
	end
	redraw = true
end
...
local function gridmenuKeyHandler(_, _, args, _, isRepeat)
	...
	if builderIsFactory then
		if meta and WG.Quotas then -- handle quotas IMPORTANT: USE SPACE LATER ON
			local change = 1
			if shift then
				change = 5
			end
...
local function drawCell(rect)
	...
	local quotanr
	if WG.Quotas and WG.Quotas.getQuotas()[uid] then
		quotanr = WG.Quotas.getQuotas()[uid].amount
	end
	...
	if quotanr and quotanr ~= 0 then
		local queueFontSize = cellInnerSize * 0.29
		local textPad = math_floor(cellInnerSize * 0.1)
		local textWidth = font2:GetTextWidth(quotanr) * queueFontSize
		RectRound(
			rect.x,
			rect.y + cellPadding + iconPadding,
			rect.x + textWidth + (textPad * 2), -- double pad, for a pad at the start and end
